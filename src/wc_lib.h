/**	
	...
	@note
		- C++11 표준으로 작성하는 것이었나 익숙하지 않은 관계로 실제 구현은 C++11 스럽지않다. C++11이 익숙해지면 리펙토링이 필요하다.
		- boost 사용 여부를 고민하다가 일단 일부를 포함시켰다. 향후 asio, thread 등을 boost로 전환하는 것을 고려하자.
...
#define WC_TRACE(x) x
/// 디버깅 블럭
#define WC_TRACE_BLOCK(x) {x} 
/// 디버깅 로그
#define WC_TRACE_WRITE WCLOG_WRITE
/// 디버깅 로그
#define WC_TRACE_WRITEFORMAT WCLOG_WRITEFORMAT
#else
/// 디버깅
#define WC_TRACE(x) 
/// 디버깅 블럭
#define WC_TRACE_BLOCK(x) 
/// 디버깅 로그
#define WC_TRACE_WRITE(...)  
/// 디버깅 로그
#define WC_TRACE_WRITEFORMAT(...)  
#endif
*/
/// Lib로그 열기
...
/// Lib로그 쓰기
#define WCLOG_WRITECONSOLE ge_Log.WriteConsole
/// Lib로그 쓰기
#define WCLOG_WRITE ge_Log.Write
/// Lib로그 쓰기
#define WCLOG_WRITEFORMAT ge_Log.WriteFormat
/// Lib로그 닫기
#define WCLOG_CLOSE ge_Log.Close

/// malloc 해제
#define WC_FREE(x) { if ((x) != NULL) free((x)); (x) = NULL; }
/// new 해제
#define WC_DELETE(x) { if ((x) != NULL) delete((x)); (x) = NULL; }
/// new[] 해제
#define WC_DELETE_ARRAY(x) { if ((x) != NULL) delete[] ((x)); (x) = NULL; }
/// new WCObject 해제
#define WC_DELETE_WC(x) { if ((x)!=NULL && x->IsLive()) delete((x)); (x) = NULL; }
...
@return 실패: 0	
*/
inline unsigned long long WC_CreateObjectKey()
{
	// 객체키 설정
	struct timespec tspecKey;
	if (clock_gettime(CLOCK_REALTIME, &tspecKey) == -1) {
		WCLOG_WRITE(WCLog::E_LEVEL_ERROR, str(boost::format("[%s:%s:%d] Function -> clock_gettime (%d, %s)") % __FILE__ % __FUNCTION__ % __LINE__ % errno % strerror(errno)));
		return WC_OK;
	}
	else {
